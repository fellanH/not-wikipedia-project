...
function detectLogLevel(line) {
  ...
  // Error patterns
  if (/\b(error|exception|fatal|fail|failed|✗|❌|critical)\b/i.test(line) ||
      /error:\s/i.test(line) ||
  ...
  // Warning patterns
  if (/\b(warn|warning|⚠|⚠️|deprecated|deprecation)\b/i.test(line) ||
      /warning:\s/i.test(line)) {
    return 'warn';
  }
  
  // Success patterns
  if (/\b(success|successful|✓|✅|completed|done|finished)\b/i.test(line) ||
      /successfully/i.test(line)) {
    return 'success';
  }
  
  // Info patterns
  if (/\b(info|information|→|ℹ|ℹ️|notice)\b/i.test(line) ||
      /info:\s/i.test(line) ||
      /→\s/.test(line)) {
    return 'info';
  }
...
function appendLog(type, text) {
  ...
  lines.forEach(line => {
    if (line.trim() || line === '') {
      ...
      const detectedType = type === 'stdout' || type === 'stderr' 
        ? detectLogLevel(line) 
        : type;
...
function calculateVisibleRange() {
  ...
  const endLine = Math.min(totalLines, startLine + linesPerViewport + (bufferLines * 2));
...
function renderLogs() {
  ...
  logsToRender.forEach((log, localIndex) => {
    ...
    div.setAttribute('aria-label', `Log line ${globalIndex + 1}, ${log.type} level`);
    ...
    if (log.taskIds && log.taskIds.length > 0) {
      log.taskIds.forEach(taskId => {
        const taskIdPattern = new RegExp(`(${taskId.replace(/\./g, '\\.')})`, 'g');
        lineText = lineText.replace(taskIdPattern, '<span class="task-id-link" data-task-id="$1" role="button" tabindex="0" aria-label="Filter logs for task $1">$1</span>');
      });
    }
    
    // Highlight URLs
    const urlPattern = /(https?:\/\/[^\s]+)/g;
    lineText = lineText.replace(urlPattern, '<a href="$1" target="_blank" rel="noopener noreferrer" class="log-url">$1</a>');
    
    // Highlight file paths
    const filePathPattern = /([\/~][^\s]*\.[a-zA-Z]{2,4})/g;
    lineText = lineText.replace(filePathPattern, '<span class="log-filepath">$1</span>');
    ...
    if (logSearchTerm && logSearchTerm.length > 0) {
      try {
        ...
        const searchLower = logSearchCaseSensitive ? logSearchTerm : logSearchTerm.toLowerCase();
        const lineLower = logSearchCaseSensitive ? log.line : log.line.toLowerCase();
  ...
  if (searchMatchIndex >= 0 && searchMatches.length > 0) {
    ...
    const matchElement = logsContent.querySelector(`[data-global-index="${actualIndex}"]`);
    if (matchElement) {
...
function exportLogs(format = 'txt') {
  ...
  switch (format) {
    ...
    case 'csv': {
      const csvHeaders = ['Timestamp', 'Type', 'Level', 'Line', 'Task IDs', 'Bookmarked'];
      const csvRows = reversedLogs.map(log => {
        const taskIds = (log.taskIds || []).join('; ');
        const bookmarked = logBookmarks.has(logBuffer.indexOf(log)) ? 'Yes' : 'No';
        return [
...
function saveBookmarks() {
  try {
    localStorage.setItem('logBookmarks', JSON.stringify(Array.from(logBookmarks)));
  } catch (err) {
...
logsContent.addEventListener('scroll', debounce(() => {
  if (virtualScrollEnabled) {
    renderLogs();
  }
}, 50));
...
document.addEventListener('keydown', (e) => {
  ...
  if (e.target.tagName === 'INPUT' || e.target.tagName === 'TEXTAREA') {
    ...
    if ((e.ctrlKey || e.metaKey) && (e.key === 'g' || e.key === 'G' || e.key === 'f' || e.key === 'F')) {
  ...
  if ((e.ctrlKey || e.metaKey) && e.shiftKey && e.key === 'G' && isLogsTab) {
    e.preventDefault();
    if (searchMatches.length > 0) {
      searchMatchIndex = searchMatchIndex <= 0 ? searchMatches.length - 1 : searchMatchIndex - 1;
      renderLogs();
    }
  }
  ...
  if (e.key === 'f' && isLogsTab && !e.ctrlKey && !e.metaKey && !e.shiftKey && !e.altKey) {
    e.preventDefault();
    btnFollowMode.click();
  }
  
  // Ctrl/Cmd + K: Clear logs (when in logs tab and search is focused)
  if ((e.ctrlKey || e.metaKey) && e.key === 'k' && isLogsTab && document.activeElement === logSearch) {
...
function displayWorkerTimeline(data) {
  ...
  timelinePanel.innerHTML = `
    ...
    <div>
      ${mergedLogs.slice(0, 100).map(log => `
